      ...
      <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
        {plans.map((p) => (
          <article key={p.id} id={p.slug} className="card flex scroll-mt-24 flex-col">
            ...
            <p className="mb-4 text-xs text-muted">{p.termMonths ? `${p.termMonths} monthly payments, then it ends` : "Cancel anytime"}{p.grantsPremium ? " · Includes premium membership" : ""}</p>
